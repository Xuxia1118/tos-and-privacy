...
REDIRECT_URI = os.getenv("DISCORD_REDIRECT_URI")  # Discord Developer Portal 完全一致
...
def login():
    ...
    params = {
        ...
        "response_type": "code",
        "scope": OAUTH_SCOPE.replace(" ", "+")  # Discord OAuth2 需要用 + 分隔
    }
...
def settings():
    ...
    html = """
    <form method="POST">
        Prefix: <input name="prefix" value="{{prefix}}"><br>
        歡迎頻道 ID: <input name="welcome_channel_id" value="{{welcome_channel_id}}"><br>
        歡迎訊息: <textarea name="welcome_message">{{welcome_message}}</textarea><br>
